...
def _ensure_brackets(client, account_id: str, symbol: str, lifecycle, log_prefix: str = '') -> None:
    ...
    try:
        ...
        if not lifecycle.stop_order_id and lifecycle.stop is not None:
            stop_oid = client.place_equity_stop_order(
                ...
                symbol=symbol,
                action='SELL',
                quantity=int(lifecycle.filled_qty),
...
def handle_alert_for_autoexec(
    ...
    in_window = _in_exec_window(now, allow_opening, allow_midday, allow_power)
